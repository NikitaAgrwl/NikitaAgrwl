...
export default function MovieDetails({ selectedMovie, lang }) {
  ...
  const getMovieReview = useCallback(async () => {
    try {
      const reviewResponse = await fetch(`/api/${lang}/movieReviews/${id}`);
      const reviewJson = await reviewResponse.json();
      if (reviewJson.status_code > 1) throw new Error('The resource you requested could not be found.')
      setMovieReview(reviewJson)
      } catch (e) {
      console.error(e)
    } 
  } , [lang, id])
  ...
  useEffect(() => {
    getTmdbApi()
    getMovieReview()
  }, [lang, getTmdbApi, getMovieReview])
  ...
  const getCast = () => {
   ...
   const getReviews = () => {
     ...
     const movieReview = reviewData.results;
     return (movieReview.slice(0, 10).map (reviewMember => {
     return (
       <Col md="auto">
         <Card className = "movie-card" >
           <Card.Img className = "movie-image" variant="top" src={imagePath + reviewMember.author_details.avatar_path}/>
           <Card.Body>
             <Card.Title className = "movie-title">{reviewMember.username}</Card.Title>
             <Card.Text className = "movie-content"> {reviewMember.content}
             </Card.Text>
           </Card.Body>
         </Card>
       </Col>
