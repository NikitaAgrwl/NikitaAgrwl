...
const tmdbApiKey = process.env.TMDB_API_KEY || '0c1e1b29fb2333268c86f9f0e743af6b'
...
function endpointCreation() {
  try {
    ...
    app.get('/api/:lang/movieReviews/:tmdbId', async (req, res) => {
      const id = req.params.tmdbId
      const lang = req.params.lang
      optionsMovieReview.qs.language = lang
      optionsMovieReview.url = `https://api.themoviedb.org/3/movie/${id}/reviews`
      res.json(await apiCall(optionsMovieReview))
